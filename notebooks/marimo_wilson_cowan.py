...
def _():
    ...
    return (mo, PhasePlaneWidget)


@app.cell

def _(mo):
    c_slider = mo.ui.slider(0.1, 10.0, 0.1, value=2.5, label="c (excitatory gain)")
    mu_slider = mo.ui.slider(0.1, 5.0, 0.1, value=1.0, label="μ (inhibitory gain)")
    tmax_slider = mo.ui.slider(10, 500, 10, value=100, label="t_max")
...
def _(PhasePlaneWidget, c_slider, mo, mu_slider, tmax_slider):
    ppw = PhasePlaneWidget(
        model="wilson_cowan",
        params={"c": float(c_slider.value), "mu": float(mu_slider.value)},
